...
class Ithenticate
{
    ...
    private $sid;
	// These properties are for communicating via proxy
	private $proxyName;
	private $proxyPort;
     ...
     */
    public function __construct($username, $password, $proxyObject = null)
    {
	error_log("JoelFanIthenticate: getting parameters ".$proxyObject->proxyName." ".$proxyObject->proxyPort);
	if (isset($proxyObject)) {
		error_log("JoelFanIthenticate: assigning proxyObject ".$proxyObject->proxyName." ".$proxyObject->proxyPort);
	...
	error_log("JoelFanIthenticate: check Proxy ".$this->proxyName." ".$this->proxyPort);
		// Add proxy support // 
		if (isset($this->proxyName)) {
     ...
     * Create a new folder
     */
    public function createFolder($folder_name, $folder_description, $group_id, $exclude_quotes, $add_to_index = 1)
    {
        $client = new Client($this->getUrl());
        $args = array(
            'sid' => new Value($this->getSid()),
	...
	public function addUser($userEmail, $userPass, $firstName, $lastName)
    {
        $client = new Client($this->getUrl());
        $args = array(
            'sid' => new Value($this->getSid()),
            'email' => new Value($userEmail),
            'password' => new Value($userPass),
    ...
    */	
	...
	/************************************************************************************/
	/* End of proposed user management 	joelfan dario.basset@unimi.it					*/
	/* 2022 06 11																		*/
	/************************************************************************************/
